#!/usr/bin/env python3
"""
Foundry Evolution V5 — Mean Reversion Deep Dive
...
RESULTS_DIR = RESEARCH_DIR / "runs" / "evolution_v5"
...
AUFGABE: Generiere {n_candidates} MEAN-REVERSION-Kandidaten. Fokussiert auf BB+RSI+EMA-Raum!

BENCHMARK (zu VERBESSERN):
V17 Mean_Reversion_BB: Score 4.88, 5/6 profitabel, Avg R +3.88%, DD 5.66%, CL max 8
Entry: close < bb_lower_20 AND rsi_14 < 30 AND close > ema_200
Exit: Trail 1.5%, SL 3.0%, Max Hold 36h

⚠️ V17 ist IS-stark aber WF-SCHWACH (OOS 0/6 profitabel, Robustness 30/100).
Du musst V17 VERBESSERN — nicht kopieren!

FOKUS: Mean Reversion auf 1h-Crypto. V17 benutzt BB+RSI+EMA200.
Probiere:

1. **BB-Parameter variieren**: bb_14 statt bb_20, bb_30, bb_lower mit rsi < 35 (nicht 30)
2. **RSI-Schwellen**: rsi_14 < 25 (strenger), rsi_7 < 20 (schneller RSI)
3. **EMA-Filter**: ema_100 statt ema_200, ema_50 als Short-Trend
4. **Zusätzliche Filter**: volume > sma_20_volume, bb_width_20 < threshold (Squeeze)
5. **Exit-Optimierung**: Trail 2.0% statt 1.5%, Max Hold 24h statt 36h, SL 2.5%
6. **Kombinationen**: BB+RSI+EMA+SMA als Doppel-Filter
7. **Andere Mean-Reversion-Indikatoren**: ZScore < -1.5, Stochastic %K < 20

WICHTIGE REGELN:
- NIEMALS period=0 oder period=1! Minimum period=2
- entry_condition MUSS konkreter DSL-Ausdruck sein
- Indikator-Namen MÜSSEN mit period-Nummer matchen
- IMMER exit_config mit trailing_stop_pct + stop_loss_pct + max_hold_bars
- Max 4 Indikatoren pro Strategie
- Nur LONG
- JEDER Kandidat MUSS Mean-Reversion sein (nicht Trend/Momentum!)

GÜLTIGE INDIKATOREN:
SMA(N), EMA(N), RSI(N), BB(N, std_dev=2.0), ATR(N), ZSCORE(N)
Sonder-Variablen: bb_upper_N, bb_lower_N, bb_mid_N, bb_width_N, close, open, high, low, volume
...
[
  {{
    ...
    "entry_condition": "close < bb_lower_20 AND rsi_14 < 35 AND close > ema_100",
    "exit_config": {{
...
def load_previous_hall_of_fame() -> list:
    ...
    best = []
    # Check all evolution_v5 result files
    if RESULTS_DIR.exists():
        for f in sorted(RESULTS_DIR.glob("evolution_v5_results*.json")):
            try:
                with open(f) as fh:
                    data = json.load(fh)
...
def run_evolution_v5(n_iterations: int = 15, candidates_per_iter: int = 3):
    print("=" * 70)
    print("EVOLUTION V4 — Creative Explosion")
    print(f"Temp: 0.3 | Iterations: {n_iterations} | Candidates/Iter: {candidates_per_iter}")
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        # Call LLM
        try:
            print(f"  🤖 Calling {MODEL} (temp=0.3)...")
            resp = call_llm(prompt, temperature=0.3)
    ...
    # Save results
    results_file = RESULTS_DIR / "evolution_v5_results.json"
...
if __name__ == "__main__":
    ...
    run_evolution_v5(n_iterations=args.iterations, candidates_per_iter=args.candidates)