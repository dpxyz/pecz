"""Binance Long/Short Account Ratio collector."""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt == MAX_RETRIES - 1:
                logger.error("BN LS fetch failed for %s: %s", symbol, e)
                return []
            await asyncio.sleep(RETRY_BASE_DELAY * (2 ** attempt))
    return []
...
async def fetch_historical(session: aiohttp.ClientSession, semaphore: asyncio.Semaphore) -> pl.DataFrame:
    days = BACKFILL_DAYS["binance_ls_ratio"]
    ...
    return pl.DataFrame(all_rows) if all_rows else pl.DataFrame(schema={
        ...
        "long_account": pl.Float64, "short_account": pl.Float64, "long_short_ratio": pl.Float64,
    })
